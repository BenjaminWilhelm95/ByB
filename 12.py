...
# Función para enviar comandos G-code (versión manual específica)
def enviar_comando_manual(comando, x=0, y=0):
    global x_pos, y_pos
    if ser and ser.is_open:
        comando_completo = comando + '\n'
        ser.write(comando_completo.encode())
        ...
        # Corregimos las posiciones según el movimiento
        # Ahora sumamos el valor real, sin invertir los ejes (X suma + cuando se mueve a la derecha, Y suma + cuando se mueve hacia arriba)
        x_pos += x
        y_pos += -y

        # Actualizar las etiquetas del GUI con los valores corregidos
        label_x.config(text=f"Posición X: {x_pos}")
        label_y.config(text=f"Posición Y: {y_pos}")
    else:
        messagebox.showwarning("Advertencia", "No hay conexión con el puerto serial.")
...
entry_manual_x.grid(row=1, column=2)
entry_manual_x.insert(0, "0")

entry_manual_y = tk.Entry(root)
entry_manual_y.grid(row=0, column=2)
...
# Botones para mover el eje X
def mover_x_izquierda():  # Debería mover X hacia la izquierda
    mover_distancia_x(int(entry_manual_x.get()))  # -X

def mover_x_derecha():  # Debería mover X hacia la derecha
    mover_distancia_x(-int(entry_manual_x.get()))  # +X

# Botones para mover el eje Y
def mover_y_abajo():  # Debería mover Y hacia abajo
    mover_distancia_y(-int(entry_manual_y.get()))  # -Y

def mover_y_arriba():  # Debería mover Y hacia arriba
    mover_distancia_y(int(entry_manual_y.get()))  # +Y

btn_x_mas = ttk.Button(root, bootstyle="black_link", text="+", command=mover_y_arriba)
btn_x_mas.grid(row=0, column=3)

btn_x_menos = ttk.Button(root, bootstyle="black_link", text="-", command=mover_y_abajo)
btn_x_menos.grid(row=0, column=1)

btn_y_mas = ttk.Button(root, bootstyle="dark_link", text="+", command=mover_x_derecha)
btn_y_mas.grid(row=1, column=3, padx=10, pady=10)

btn_y_menos = ttk.Button(root,bootstyle="dark_link", text="-", command=mover_x_izquierda)
...
# Botones de Eje Y
    ...
    ttk.Button(interval_window, text="   Y  ", bootstyle="dark_outline", command=y1_positivo).grid(row=4, column=3, padx=0, pady=5)
    ttk.Button(interval_window, text="  -Y  ", bootstyle="dark_outline", command=y1_negativo).grid(row=6, column=3, padx=0, pady=5)
    ...
    # Botones de Eje X
    ttk.Button(interval_window, text="  10X ", bootstyle="dark_outline", command=x100_positivo).grid(row=5, column=6, padx=5, pady=0)
    ttk.Button(interval_window, text="   5X ", bootstyle="dark_outline", command=x10_positivo).grid(row=5, column=5, padx=5, pady=0)
    ttk.Button(interval_window, text="   X  ", bootstyle="dark_outline", command=x1_positivo).grid(row=5, column=4, padx=5, pady=0)
    ttk.Button(interval_window, text="  -X  ", bootstyle="dark_outline", command=x1_negativo).grid(row=5, column=2, padx=5, pady=0)
    ttk.Button(interval_window, text="  -5X ", bootstyle="dark_outline", command=x10_negativo).grid(row=5, column=1, padx=5, pady=0)
    ttk.Button(interval_window, text=" -10X ", bootstyle="dark_outline", command=x100_negativo).grid(row=5, column=0, padx=5, pady=0)
...
ttk.Label(root, text="Segundos entre movimiento:").grid(row=2, column=6, padx=10, pady=5)
...
#Arreglar unidades con los mm acordados, arreglar abrir coordenadas
#Arreglar movimiento manual de los Entry
#Cambiar todo a mm
#Arreglar Labels de movimiento manual e interfaz principal
# Y=12 cm aprox ; X=20 cm
# Ajustar los intervalos a 1mm 2mm y 5mm